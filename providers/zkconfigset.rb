...
action :create do
  converge_by("create config set #{new_resource.configset_name} if missing \n") do
    ...
    execute "zk_config_set_upconfig_#{new_resource.configset_name}_update_upload" do
      command "/usr/local/solr_zkconfigsets/solr/server/scripts/cloud-scripts/zkcli.sh -zkhost #{new_resource.zkhost} -cmd upconfig -confdir #{::File.join(new_resource.zkconfigsets_home, new_resource.configset_name, 'conf')} -confname #{new_resource.configset_name} 2>&1"
      action :nothing
